...
public class RetrospectiveApiController {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Returned all available retrospectives from the server", content = {
                                        @io.swagger.v3.oas.annotations.media.Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @io.swagger.v3.oas.annotations.media.Schema(implementation = Retrospective.class)),
        ...
        public ResponseEntity<Page<Retrospective>> getAllRetrospectives(
                ...
                logger.info(String.format("Attempting to get all retrospectives with pagination page=%d, pageSize=%d",
                                page, pageSize));
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Returned all available retrospectives for given date from the server", content = {
                                        @Content(mediaType = "application/json", schema = @Schema(implementation = Retrospective.class)) }),
        ...
        @GetMapping("/filter")
        public ResponseEntity<List<Retrospective>> getAllRetrospectivesByDate(
                        @Parameter(description = "Date to filter retrospectives by") @RequestParam("date") @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date) {
                logger.info(String.format("API invoked: httpMethod=GET, path=retrospectives/filter?{date} (date=%s)",
                                date.toString()));
                List<Retrospective> retrospectives = retrospectiveService.getRetrospectivesByDate(date);

                if (!retrospectives.isEmpty()) {
                        logger.info("Successfully found all retrospectives with date: " + date
                                        + ". Returning status OK");
                        return ResponseEntity.ok(retrospectives);
                } else {
        ...
        @Operation(summary = "Create a new retrospective", description = "Endpoint to create a new retrospective to be updated in the server")
        @ApiResponses(value = {
        ...
        public ResponseEntity<String> createNewRetrospective(
                        @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "New retrospective to be created.", required = true, content = @Content(schema = @Schema(implementation = Retrospective.class))) @RequestBody final Retrospective retrospective) {
                ...
                try {
                        ...
                        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                        .body("Unable to create new item. Process failed with exception: "
                                                        + exp.getMessage());
                }
        }
        ...
        public ResponseEntity<Void> addFeedbackToRetrospective(
                        ...
                        @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "New feedback to be created for retrospective.", required = true, content = @Content(schema = @Schema(implementation = FeedbackItem.class))) @RequestBody FeedbackItem newFeedbackItem) {
                logger.info(String.format("API Invoked: httpMethod=PUT path='/retrospective/%s/feedback'",
                                retrospectiveName));
                logger.info("Attempting to add new feedback for retrospective with values: " + newFeedbackItem);

                try {
                        logger.info(String.format(
                                        "Feedback for retrospective '%s' successfully added. Returning status OK",
                                        retrospectiveName));
        ...
        public ResponseEntity<Void> updateFeedbackItem(
                        ...
                        @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Retrospective feedback to be edited", required = true, content = @Content(schema = @Schema(implementation = FeedbackItem.class))) @RequestBody FeedbackItem feedbackItem) {
                logger.info(String.format("API Invoked: httpMethod=PUT path='/retrospective/%s/feedback/%d'",
                                retrospectiveName, itemId));
                logger.info(String.format("Updating feedback for retrospective: %s, item ID: %d", retrospectiveName,
                                itemId));
                logger.info("Attempting to update feedback for retrospective with values: " + feedbackItem);

                try {
                        logger.info(String.format(
                                        "Feedback for retrospective '%s', feedback item '%d' successfully updated. Returning status OK",
                                        retrospectiveName, itemId));
                        retrospectiveService.updateFeedbackForRetrospective(retrospectiveName, itemId, feedbackItem);
                        return ResponseEntity.ok().build();
                } catch (IllegalArgumentException exp) {
                        logger.warn("Feedback item not found for retrospective: " + retrospectiveName + ", item ID: "
                                        + itemId);
                        return ResponseEntity.notFound().build();
                } catch (Exception exp) {
                        logger.error("Failed to update feedback for retrospective: " + retrospectiveName + ", item ID: "
                                        + itemId, exp);
                        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
                }
        }

        @Operation(summary = "Create task for existing retrospective")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "New task successfully added to retrospective", content = @Content),
                        @ApiResponse(responseCode = "400", description = "Invalid request body", content = @Content),
                        @ApiResponse(responseCode = "404", description = "Retrospective not found", content = @Content),
                        @ApiResponse(responseCode = "500", description = "Failed to create new task for retrospective in server", content = @Content)
        })
        ...
        public ResponseEntity<Void> addTaskToRetrospective(
                ...
                try {
                        ...
                        logger.error("Failed to add task to retrospective: " + retrospectiveName, exp);
                        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
                }
        }

        @Operation(summary = "Update task for retrospective")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Task item successfully updated", content = @Content),
                        @ApiResponse(responseCode = "400", description = "Invalid request body", content = @Content),
                        @ApiResponse(responseCode = "404", description = "Task item not found", content = @Content),
                        @ApiResponse(responseCode = "500", description = "Failed to update task in server", content = @Content)
        })
        @PutMapping("{retrospectiveName}/task/{itemId}")
        public ResponseEntity<Void> updateTaskItem(
                        @PathVariable String retrospectiveName,
                        @PathVariable int itemId,
                        @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Retrospective task to be edited", required = true, content = @Content(schema = @Schema(implementation = TaskItem.class))) @RequestBody TaskItem taskItem) {
                logger.info(String.format("API Invoked: httpMethod=PUT path='/retrospective/%s/task/%d'",
                                retrospectiveName, itemId));
                logger.info(String.format("Updating task for retrospective: %s, item ID: %d", retrospectiveName,
                                itemId));
                logger.info("Attempting to update task for retrospective with values: " + taskItem);

                try {
                        retrospectiveService.updateTaskForRetrospective(retrospectiveName, itemId, taskItem);
                        return ResponseEntity.ok().build();
                } catch (IllegalArgumentException exp) {
                        logger.warn("Task item not found for retrospective: " + retrospectiveName + ", item ID: "
                                        + itemId);
                        return ResponseEntity.notFound().build();
                } catch (Exception exp) {
                        logger.error("Failed to update task for retrospective: " + retrospectiveName + ", item ID: "
                                        + itemId, exp);
                        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
                }
        }

        @Operation(summary = "Get all tasks for a retrospective")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Returned all tasks for the retrospective", content = @Content),
                        @ApiResponse(responseCode = "404", description = "No tasks found for the retrospective", content = @Content),
